...
function TodoListComponent() {
    ...
    const [stateEditInputValue, setEditInputValue] = useState('')
    // ID edit
    const [stateIdEditItem, setIdEditItem] = useState('')
  
    const [stateTodoList, setTodoList] = useState([...TODOLIST]);
    const [stateWorkDone, setWorkDone] = useState([...WORKDONE]);
    const [statefeedback, setFeedback] = useState("");
    // Trạng thái đang chỉnh sửa hoặc đã sửa
    // const [state_Editting, set_Editting] = useState('fas fa-edit')
    // nguyen
